...
#              under SSP126 scenario. It processes land cover, elevation, GPP, 
...
# Description: Extracts environmental variables for a specific year and month
# Parameters:
...
extract_environmental_variables <- function(year, month, pg_buffer) {
  ...
  geom_time_gpp <- NULL
  gpp_file <- paste0('../predictor/GPP_mon_ssp126_', year, '.tif')
  gpp <- rast(gpp_file)
  gpp <- gpp[[month]]
  ...
  geom_time_temp <- NULL
  temp_file <- paste0('../predictor/EC-Earth3_ssp126_tmp-', year, '.tif')
  temp <- rast(temp_file)
  temp <- temp[[month]]
  ...
  geom_time_pre <- NULL
  pre_file <- paste0('../predictor/EC-Earth3_ssp126_pre-', year, '.tif')
  pre <- rast(pre_file)
  pre <- pre[[month]]
  ...
  geom_time_rh <- NULL
  rh_file <- paste0('../predictor/hurs_Amon_EC-Earth3_ssp126_r1i1p1f1_gr_', 
                    year, '01-', year, '12.tif')
  rh <- rast(rh_file)
  rh <- rh[[month]]
...
for (year in target_years) {
  for (month in 1:12) {
    ...
    output_file <- paste0('ssp126_env_variables_', year, '_', 
