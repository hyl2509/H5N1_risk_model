...
#              under SSP585 scenario. It processes land cover, elevation, GPP, 
...
# Description: Extracts environmental variables for a specific year and month under SSP585 scenario
# Parameters:
...
extract_environmental_variables <- function(year, month, pg_buffer) {
  ...
  geom_time_gpp <- NULL
  gpp_file <- paste0('../predictor/GPP_mon_ssp585_', year, '.tif')
  gpp <- rast(gpp_file)
  gpp <- gpp[[month]]
  ...
  geom_time_temp <- NULL
  temp_file <- paste0('../predictor/EC-Earth3_ssp585_tmp-30s-', year, '.tif')
  temp <- rast(temp_file)
  temp <- temp[[month]]
  ...
  geom_time_pre <- NULL
  pre_file <- paste0('../predictor/EC-Earth3_ssp585_pre-30s-', year, '.tif')
  pre <- rast(pre_file)
  pre <- pre[[month]]
  ...
  geom_time_rh <- NULL
  rh_file <- paste0('../predictor/hurs_Amon_CNRM-CM6-1-HR_ssp585_r1i1p1f2_gr_', 
                    year, '01-', year, '12.tif')
  rh <- rast(rh_file)
  rh <- rh[[month]]
